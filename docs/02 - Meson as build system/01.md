...
Now, GtkApplication takes two parameters in construction, in this case `string application_id` and `ApplicationFlags flags`. Let's explain each one:
...
Now, you can compile with `ninja` in your `builddir` and run it!. As you can see, we have a warning, that tell us we have to implement the `activate` method in our app class.
